...
import {
  buildAgent as buildRelationshipDeskAgent,
  configureAgentsSdk,
  run as runAgent,
  type AgentContext,
} from '../agent/relationshipdesk.js';
...
export async function streamAgentTurn(args: {
  ...
  modelError: import('../agent/relationshipdesk.js').ModelErrorDetail | null;
}> {
  ...
  // Captured by the OpenAI fetch shim in relationshipdesk.ts on any non-2xx
  ...
  const modelError: { current: import('../agent/relationshipdesk.js').ModelErrorDetail | null } = {
    current: null,
  };
  ...
  try {
    ...
    const agent = buildRelationshipDeskAgent(ctx);
